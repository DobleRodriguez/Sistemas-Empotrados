...
int32_t uart_init (uart_id_t uart, uint32_t br, const char *name)
{
	...
	/* Programamos cuándo generar las interrupciones */
	...
	/* En un driver L0 no hacemos chequeo de errores */
	return 0;
}
...
ssize_t uart_send (uint32_t uart, char *buf, size_t count)
{
	...
	uart_regs[uart]->mTxR = 1; /* Comienzo de la región crítica */
	...
	/* Desenmascaramos las interrupciones de,l transmisor */
...
ssize_t uart_receive (uint32_t uart, char *buf, size_t count)
{
	...
	/* Leemos del búfer circular los bytes que podamos */
...
static inline void uart_isr (uart_id_t uart)
{
	...
	if(uart_regs[uart]->RxRdy) {
		...
		/* Si el búfer circular está lleno no podemos aceptar más datos */
	...
	if (uart_regs[uart]->TxRdy) {
		...
		/* Si el búfer está vacío es que no hay más datos */
