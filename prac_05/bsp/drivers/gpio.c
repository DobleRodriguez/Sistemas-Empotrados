...
typedef struct
{
	...
	uint32_t PU_EN[2];			/*	Habilitación del pull-up/pull-down					*/
	/* uint32_t FUNC_SEL[4] */
	uint64_t FUNC_SEL[2];		/*	Selección del modo de funcionamiento de cada pin	*/
...
{
	*pin_data = (gpio_regs->DATA[(pin >> 5) & 1] >> (pin & 0x1f)) & 1;
	return gpio_no_error;
}
...
inline gpio_err_t gpio_set_port_func (gpio_port_t port, gpio_func_t func, uint32_t mask)
{
	...
	for (uint32_t i = 0; i < 32; i++) {
		if ((mask >> i) & 1) {
			gpio_regs->FUNC_SEL[port] &= ~(3 << i); 
			gpio_regs->FUNC_SEL[port] |= func << i;
		}
	}
	return gpio_no_error;
}
...
{
	// gpio_regs->FUNC_SEL[(pin >> 4) & 3] = func << (pin & 0xf);
	gpio_regs->FUNC_SEL[(pin >> 5) & 1] = func << (pin & 0xf);
	return gpio_no_error;
}
